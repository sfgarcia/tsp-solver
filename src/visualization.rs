...
pub struct Visualization<'a> {
    ...
    pub min_y: f32,
    pub max_y: f32,
    pub root: &'a DrawingArea<BitMapBackend<'a, RGBPixel>, Shift>,
    pub chart: ChartContext<'a, BitMapBackend<'a, RGBPixel>, Cartesian2d<RangedCoordf32, RangedCoordf32>>,
    route: Vec<Node>,
}

impl<'a> Visualization<'a> {
    pub fn new(path: &'a str, width: u32, height: u32, margin: u32, route: Vec<Node>) -> Self {
        ...
        let chart = ChartBuilder::on(&root)
            ...
            .unwrap();
        Self { width, height, margin, drawing_width: width - 2 * margin, drawing_height: height - 2 * margin, min_x, max_x, min_y, max_y, root: &root, chart, route }
    }
}
